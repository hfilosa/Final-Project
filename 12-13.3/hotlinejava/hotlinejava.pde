...
void bulletCollisionOLD(){
  for (int j=0;j<bullets.size();j++){
    ...
    //problem area ran one test that came up positive but crashed program 
    for (int i=4;i<walls.size();i++){
      if (collision(bullets.get(j).getCoors(),walls.get(i).getCoors())){
        walls.remove(i);
        println("Wall Hit!");
        bullets.remove(j);
        break;
...
void bulletCollision(){
    for(Bullet Bill: bullets){
      if (wallCollision((Bill.getCoors()))){
        println("Wall Hit");
        bullets.remove(Bill);
    }
  }
